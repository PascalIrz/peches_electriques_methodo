...
data_simp <- data %>%
  select(meth = type_peche, anodes, dist = dist_anode_cathode, lar_mo = larg_moy, pro_mo = prof_moy,
         ...
         temp, volt = voltage, puiss = puissance, intens = intensite, pen1 = penny_1m,
         pen1.5 = penny_1.5m, cran)
...
cor_tot <- data_simp %>%
  select(cond, cond25, temp, volt, puiss, intens, pen1, pen1.5) #%>%
...
cor_2020 <- data_simp %>%
  filter(annee == 2020) %>% 
  mutate(intens = ifelse(is.na(intens) & !is.na(volt), 1000 * puiss / volt)) %>% 
  select(cond, cond25, temp, volt, puiss, intens, pen1, pen1.5) %>% 
  select(cor_tot_order)
...
labels <- map(.x = (2:4), .f = model_simp, df = prov, cond = "conductivite_25")
...
comp_penny_plot <- data %>%
  select(starts_with("penny")) %>%
  pivot_longer(cols=1:2) %>%
    ggplot(aes(x=value, color = name, fill = name)) +
    geom_density(alpha = 0.2) +
    labs(x = "Mesure en V/m", y = "Densité", title = "Mesures Penny", fill = "Penny",
         caption = "Les lignes pointillées indiquent \nles valeurs moyennes à 1m et 1,5m") +
    guides(color = FALSE) +
    geom_vline(xintercept = mean(data$penny_1m, na.rm = T), col = RColorBrewer::brewer.pal(3, "Dark2")[2],
               linetype = "dashed")   +
    geom_vline(xintercept = mean(data$penny_1.5m, na.rm = T), col = RColorBrewer::brewer.pal(3, "Dark2")[1],
               linetype = "dashed") +
    annotate(geom = "text",
             x = -0.3 + mean(data$penny_1.5m, na.rm = T),
             y = 0.3,
             col = RColorBrewer::brewer.pal(3, "Dark2")[1],
             label = paste("Penny moyen à 1.5m :", round(mean(data$penny_1.5m, na.rm = T), digits = 2), "V/m"),
             angle = 90) +
  annotate(geom = "text",
           x = -0.3 + mean(data$penny_1m, na.rm = T),
           y = 0.3,
           col = RColorBrewer::brewer.pal(3, "Dark2")[2],
           label = paste("Penny moyen à 1m :", round(mean(data$penny_1m, na.rm = T), digits = 2), "V/m"),
           angle = 90) +
    scale_fill_brewer(palette = "Dark2", labels = c("1.5m", "1m")) +
